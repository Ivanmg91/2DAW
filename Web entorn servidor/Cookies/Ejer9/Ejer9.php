<?php
    ...
    $zona_anterior = $_COOKIE['zona'] ?? 'No hay datos';
    $hora_anterior = $_COOKIE['hora'] ?? 'No hay datos';

    date_default_timezone_set($zona_actual);
    $hora_actual = date("Y-m-d H:i:s");

    setcookie("zona", $zona_actual, + time() + (60 * 60 * 24 * 30), "/");
    setcookie("hora", $hora_actual, time() + (60 * 60 * 24 * 30), "/");
